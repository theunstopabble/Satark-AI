...
interface ConfidenceMeterProps {
  score: number; // 0 to 1
  isDeepfake: boolean;
  label?: string; // FIX: Optional label override for image scans
}
...
export function ConfidenceMeter({
  ...
  // FIX: Dynamic label — "REAL AUDIO" for audio, "REAL IMAGE" for images,
  // or custom label if provided
  const resultLabel =
    label || (isDeepfake ? "FAKE DETECTED" : "VERIFIED AUTHENTIC");
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 bg-card rounded-lg border shadow-sm">
      ...
      <div className={`mt-4 text-xl font-bold ${color}`}>{resultLabel}</div>
    </div>
  );
}
